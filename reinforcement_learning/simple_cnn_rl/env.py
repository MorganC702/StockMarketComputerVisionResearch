...
class TradingEnv(gym.Env):
    def __init__(
        ...
        self.start_index = 0
        self.reset_state()
        logger.info(f"[ENV INIT] {len(self.meta_df)} rows loaded.")

    def _init_csv(self):
        self.fieldnames = ["step", "timestamp", "reward", "balance", "realized_pnl",
    ...
    def set_log_suffix(self, suffix: str):
        self.log_path = self.base_log_path.with_stem(f"{self.base_log_path.stem}_{suffix}")
        self._init_csv()

    def reset_state(self):
        self.current_index = self.start_index
        self.balance = self.starting_balance
        self.position = 1  # Always start long
    ...
    def step(self, action):
        ...
        if action != self.last_action:
            ...
            fee_open = self.fee_rate * price * abs(self.position_size)
    ...
    def _get_observation(self):
        row = self.meta_df.iloc[self.current_index]
        img_path = self.root_dir / row["img"]
        if img_path.exists():
