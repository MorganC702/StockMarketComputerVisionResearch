...
class TradingEnv(gym.Env):
    def __init__(
        ...
        for tf in self.timeframes:
            img_col = f"{tf}_img"
            lbl_col = f"{tf}_lbl"
            if img_col in self.meta_df.columns:
                self.meta_df[img_col] = self.meta_df[img_col].apply(
                    lambda p: str((project_root / p).resolve()) if isinstance(p, str) else p
                )
            if lbl_col in self.meta_df.columns:
                self.meta_df[lbl_col] = self.meta_df[lbl_col].apply(
                    lambda p: str((project_root / p).resolve()) if isinstance(p, str) else p
                )

        # --- Sanity check for missing image paths ---
        missing_images = []
        ...
        if missing_images:
            msg = "\n".join([f"[Row {i}] Missing {tf}_img: {val}" for (i, tf, val) in missing_images[:10]])
            raise FileNotFoundError(f"[INIT ERROR] Missing image files:\n{msg}\n... (total missing: {len(missing_images)})")
    ...
    def step(self, action):
        ...
        reward += self.unrealized_pnl   # Reward = equity delta
    ...
    def _get_observation(self, reward):
        ...
        stacked = torch.stack(images, dim=0)  # Shape: [7, 3, H, W]
