...
from reward_functions.reward_v12 import compute_reward
...
class TradingEnv(gym.Env):
    """
    Long/short trading environment with fixed TP/SL mechanics.
    - Action space: 0 = wait, 1 = enter long, 2 = enter short
    - Auto-exit when TP/SL reached
    - Reward function uses discrete events: "tp", "sl", "hold"
    """
    ...
    def __init__(
        ...
        tp_pct=0.003,          # +0.30% take profit
        sl_pct=-0.001,         # -0.10% stop loss
        root_dir=None,
        ...
        self.action_space = gym.spaces.Discrete(3)  # 0 = wait, 1 = long, 2 = short
        ...
        self.reset_state()
        logger.info(f"[ENV INIT] Loaded {len(self.meta_df)} candles")

    def _init_csv(self):
        self.fieldnames = [
            "step", "timestamp", "event", "reward", "balance",
    ...
    def step(self, action):
        ...
        # --- Manage open position ---
            ...
            pnl_pct = pct_move if self.position == 1 else -pct_move
